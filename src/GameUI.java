...
public class GameUI extends Stage
{
	...
	public static final Stop[] SKIP_STOP = { new Stop(0, Color.web("#eeeeee")), new Stop(0.25, Color.web("#999999")), new Stop(.55, Color.web("#cdcdcd")),
	...
	public static final Image END_SCREEN_IMAGE = Tools.createImage("EndScreen.png");
	public static final Image END_SCREEN_CLICKED_IMAGE = Tools.createImage("EndScreenClicked.png");
	...
	public static final Stop[] ACTION_CIRCLE_STOP = { new Stop(0.0, Color.web("#ffffff00")), new Stop(1.0, Color.web("#ffffff55")) };

	public static final Stop[] BAD_ACTION_CIRCLE_STOP = { new Stop(0.0, Color.web("#ff000000")), new Stop(1.0, Color.web("#ff000055")) };

	public static final RadialGradient GRADIENT_CIRCLE = new RadialGradient(0, 0, .5, .5, .5, true, CycleMethod.NO_CYCLE, ACTION_CIRCLE_STOP);

	public static final RadialGradient BAD_GRADIENT_CIRCLE = new RadialGradient(0, 0, .5, .5, .5, true, CycleMethod.NO_CYCLE, BAD_ACTION_CIRCLE_STOP);
	...
	private final ImageView[] pUnit = new ImageView[MAX_UNIT_LIST];
	...
	public GameUI(int numPlayers, double widthRatio, double heightRatio, double smallestRatio)
	{
		...
		ground = new BackgroundImage(GRASSY_GROUND_IMAGE, BackgroundRepeat.REPEAT, BackgroundRepeat.REPEAT, BackgroundPosition.DEFAULT,
		...
		groundClicked = new BackgroundImage(GRASSY_GROUND_CLICKED_IMAGE, BackgroundRepeat.REPEAT, BackgroundRepeat.REPEAT, BackgroundPosition.DEFAULT,
				new BackgroundSize(GRASSY_GROUND_CLICKED_IMAGE.getWidth() * widthRatio, GRASSY_GROUND_CLICKED_IMAGE.getHeight() * heightRatio, false, false,
						false, false));
		nameSetBack = new BackgroundImage(NAME_SET_IMAGE, BackgroundRepeat.REPEAT, BackgroundRepeat.REPEAT, BackgroundPosition.DEFAULT,
				new BackgroundSize(NAME_SET_IMAGE.getWidth() * widthRatio, NAME_SET_IMAGE.getHeight() * heightRatio, false, false, false, false));
		nameSetClicked = new BackgroundImage(NAME_SET_CLICKED_IMAGE, BackgroundRepeat.REPEAT, BackgroundRepeat.REPEAT, BackgroundPosition.DEFAULT,
				new BackgroundSize(NAME_SET_CLICKED_IMAGE.getWidth() * widthRatio, NAME_SET_CLICKED_IMAGE.getHeight() * heightRatio, false, false, false,
						false));
		endScreenBack = new BackgroundImage(END_SCREEN_IMAGE, BackgroundRepeat.REPEAT, BackgroundRepeat.REPEAT, BackgroundPosition.DEFAULT,
				new BackgroundSize(END_SCREEN_IMAGE.getWidth() * widthRatio, END_SCREEN_IMAGE.getHeight() * heightRatio, false, false, false, false));
		endScreenClicked = new BackgroundImage(END_SCREEN_CLICKED_IMAGE, BackgroundRepeat.REPEAT, BackgroundRepeat.REPEAT, BackgroundPosition.DEFAULT,
				new BackgroundSize(END_SCREEN_CLICKED_IMAGE.getWidth() * widthRatio, END_SCREEN_CLICKED_IMAGE.getHeight() * heightRatio, false, false, false,
						false));
	...
	public void addUnitIcons()
	{
		for (int index = 0; index < players.get(currentPlayer).getUnitList().size(); index++)
			...
			{
				pUnit[index] = Tools.createImageView(players.get(currentPlayer).getUnitList().get(index).getIcon(), UNIT_ICON_SIZE, UNIT_ICON_SIZE, UNIT_X[index], UNIT_LIST_Y, widthRatio, heightRatio, smallestRatio, Tools.SMALL_SHADE);
	...
	public void attackClick(double mouseX, double mouseY)
	{
		...
		double bulletDirection = Math.toDegrees(Math.atan2(mouseY - unitY, mouseX - unitX));
		...
		Projectile bullet = Tools.createProjectile(currentUnit.getDamage(), BULLET_HEIGHT, BULLET_WIDTH, bulletDirection, unitX / widthRatio
				- 75 * xDist / totalDist, unitY / heightRatio - 75 * yDist / totalDist, widthRatio, heightRatio, smallestRatio, TEAM_EFFECTS[currentPlayer]);
	...
	public void createAttackButton()
	{
		...
		attackButton = Tools.createRoundedRectangle(ATTACK_BUTTON_WIDTH, BUTTON_HEIGHT, RECT_ARC_SIZE, RECT_ARC_SIZE, ATTACK_BUTTON_X, BUTTON_Y, widthRatio, heightRatio, smallestRatio, Color.WHITE, Tools.MEDIUM_OUT_SHADE);
		attackButton.setFill(GRADIENT_ATTACK);

		attackButton.setOnMousePressed(in);
		attackButton.setOnMouseReleased(out);

		attackText = Tools.createText(ATTACK_TEXT_X, TEXT_Y, widthRatio, heightRatio, "attack", Color.DARKGRAY.darker(), Tools.SMALL_SHADE, Tools.createFont("Agency FB", null, TEXT_SIZE, smallestRatio));
	...
	{
		Rectangle corpPanel = Tools.createRoundedRectangle(BUY_SIZE, BUY_SIZE, RECT_ARC_SIZE, RECT_ARC_SIZE, BUY_CORP_X, BUY_CORP_Y, widthRatio, heightRatio, smallestRatio, Tools.TRANSPARENT, null);
	...
	{
		Rectangle privatePanel = Tools.createRoundedRectangle(BUY_SIZE, BUY_SIZE, RECT_ARC_SIZE, RECT_ARC_SIZE, BUY_PRIV_X, BUY_PRIV_Y, widthRatio, heightRatio, smallestRatio, Tools.TRANSPARENT, null);
	...
	{
		Rectangle scoutPanel = Tools.createRoundedRectangle(BUY_SIZE, BUY_SIZE, RECT_ARC_SIZE, RECT_ARC_SIZE, BUY_SCOUT_X, BUY_SCOUT_Y, widthRatio, heightRatio, smallestRatio, Tools.TRANSPARENT, null);
	...
	{
		Rectangle sergPanel = Tools.createRoundedRectangle(BUY_SIZE, BUY_SIZE, RECT_ARC_SIZE, RECT_ARC_SIZE, BUY_SERG_X, BUY_SERG_Y, widthRatio, heightRatio, smallestRatio, Tools.TRANSPARENT, null);
	...
	{
		Rectangle snipPanel = Tools.createRoundedRectangle(BUY_SIZE, BUY_SIZE, RECT_ARC_SIZE, RECT_ARC_SIZE, BUY_SNIPER_X, BUY_SNIPER_Y, widthRatio, heightRatio, smallestRatio, Tools.TRANSPARENT, null);
	...
	{
		Rectangle tankPanel = Tools.createRoundedRectangle(BUY_SIZE, BUY_SIZE, RECT_ARC_SIZE, RECT_ARC_SIZE, BUY_TANK_X, BUY_TANK_Y, widthRatio, heightRatio, smallestRatio, Tools.TRANSPARENT, null);
	...
	public void createEnd(int winnerIndex)
	{
		...
		Text winner = Tools.createText(WINNER_X, WINNER_Y, widthRatio, heightRatio, players.get(winnerIndex).getName(), Color.DARKRED.darker(), Tools.SMALL_SHADE, Tools.createFont("Bookman Old Style", null, WINNER_SIZE, smallestRatio));
	...
	{
		Rectangle exitPanel = Tools.createRoundedRectangle(EXIT_PANEL_WIDTH, EXIT_PANEL_HEIGHT, RECT_ARC_SIZE, RECT_ARC_SIZE, EXIT_PANEL_X, EXIT_PANEL_Y, widthRatio, heightRatio, smallestRatio, Color.TRANSPARENT, null);
	...
	{
		Rectangle endPanel = Tools.createRoundedRectangle(END_WIDTH, END_HEIGHT, RECT_ARC_SIZE, RECT_ARC_SIZE, END_X, END_Y, widthRatio, heightRatio, smallestRatio, Tools.TRANSPARENT, null);
	...
	{
		Text minimize = Tools.createText(MIN_X, MIN_Y, widthRatio, heightRatio, "-", Color.LIGHTGRAY, Tools.SMALL_SHADE, Tools.createFont("Bookman Old Style", null, GAME_CONTROL_SIZE
				+ 10, smallestRatio));
	...
	{
		Obstacle houseOne = Tools.createSolid(Tools.HOUSE_ONE, height, width, xLoc, yLoc, widthRatio, heightRatio, smallestRatio, Tools.LARGE_OUT_SHADE);
		houseOne.updateCollShape(houseOne.getFitHeight(), houseOne.getFitWidth(), 0, houseOne.getLayoutX() + houseOne.getFitWidth() / 2, houseOne.getLayoutY()
	...
	{
		Obstacle houseTwo = Tools.createSolid(Tools.HOUSE_TWO, height, width, xLoc, yLoc, widthRatio, heightRatio, smallestRatio, Tools.LARGE_OUT_SHADE);
		houseTwo.updateCollShape(houseTwo.getFitHeight(), houseTwo.getFitWidth(), 0, houseTwo.getLayoutX() + houseTwo.getFitWidth() / 2, houseTwo.getLayoutY()
	...
	public void createMoveButton()
	{
		...
		moveButton = Tools.createRoundedRectangle(MOVE_BUTTON_WIDTH, BUTTON_HEIGHT, RECT_ARC_SIZE, RECT_ARC_SIZE, MOVE_BUTTON_X, BUTTON_Y, widthRatio, heightRatio, smallestRatio, Color.WHITE, Tools.MEDIUM_OUT_SHADE);
		moveButton.setFill(GRADIENT_MOVE);
		moveButton.setOnMousePressed(in);
		moveButton.setOnMouseReleased(out);
		moveText = Tools.createText(MOVE_TEXT_X, TEXT_Y, widthRatio, heightRatio, "move", Color.DARKGRAY.darker(), Tools.SMALL_SHADE, Tools.createFont("Agency FB", null, TEXT_SIZE, smallestRatio));
	...
	{
		Text close = Tools.createText(NAME_CLOSE_X, NAME_CLOSE_Y, widthRatio, heightRatio, "X", Tools.DARK_GREEN, Tools.SMALL_SHADE, Tools.createFont("Bookman Old Style", null, NAME_CLOSE_SIZE, smallestRatio));
	...
	{
		Rectangle okPanel = Tools.createRoundedRectangle(NAME_OK_WIDTH, NAME_OK_HEIGHT, NAME_OK_ARC_SIZE, NAME_OK_ARC_SIZE, NAME_OK_X, NAME_OK_Y, widthRatio, heightRatio, smallestRatio, Tools.TRANSPARENT, null);
	...
	{
		Obstacle rock = Tools.createSolid(Tools.ROCK, height, width, xLoc, yLoc, widthRatio, heightRatio, smallestRatio, Tools.LARGE_OUT_SHADE);
		rock.updateCollShape(rock.getFitHeight(), rock.getFitWidth(), 0, rock.getLayoutX() + rock.getFitWidth() / 2, rock.getLayoutY()
	...
	public void createSkipButton()
	{
		...
		skipButton = Tools.createRoundedRectangle(SKIP_BUTTON_WIDTH, BUTTON_HEIGHT, RECT_ARC_SIZE, RECT_ARC_SIZE, SKIP_BUTTON_X, BUTTON_Y, widthRatio, heightRatio, smallestRatio, Color.WHITE, Tools.MEDIUM_OUT_SHADE);
		skipButton.setFill(GRADIENT_SKIP);
		skipButton.setOnMousePressed(in);
		skipButton.setOnMouseReleased(out);

		skipText = Tools.createText(SKIP_TEXT_X, TEXT_Y, widthRatio, heightRatio, "skip", Color.DARKGRAY.darker(), Tools.SMALL_SHADE, Tools.createFont("Agency FB", null, TEXT_SIZE, smallestRatio));
	...
	{
		Obstacle trees = Tools.createSolid(Tools.TREE, height, width, xLoc, yLoc, widthRatio, heightRatio, smallestRatio, Tools.LARGE_OUT_SHADE);
		trees.updateCollShape(trees.getFitHeight(), trees.getFitWidth(), 0, trees.getLayoutX() + trees.getFitWidth() / 2, trees.getLayoutY()
	...
	{
		selectType = Tools.createText(TYPE_X, TYPE_Y, widthRatio, heightRatio, currentUnit.getType(), Color.GRAY, Tools.SMALL_SHADE, Tools.createFont("Agency FB", null, TYPE_SIZE, smallestRatio));
		selectHealthBar = Tools.createRoundedRectangle(HEALTH_MAX_WIDTH * currentUnit.getHealth()
				/ currentUnit.getMaxHealth(), HEALTH_MAX_HEIGHT, RECT_ARC_SIZE, RECT_ARC_SIZE, HEALTH_X, HEALTH_Y, widthRatio, heightRatio, smallestRatio, Color.RED, Tools.MEDIUM_SHADE);
		...
		selectAttRange = Tools.createText(ATTACK_RANGE_X, ATTACK_RANGE_Y, widthRatio, heightRatio, currentUnit.getAttackRange()
				+ " units", Color.GRAY, Tools.SMALL_SHADE, Tools.createFont("Agency FB", null, STAT_SIZE, smallestRatio));
		selectMoveRange = Tools.createText(MOVE_RANGE_X, MOVE_RANGE_Y, widthRatio, heightRatio, currentUnit.getMovementRange()
				+ " units", Color.GRAY, Tools.SMALL_SHADE, Tools.createFont("Agency FB", null, STAT_SIZE, smallestRatio));
		selectDamage = Tools.createText(DAMAGE_X, DAMAGE_Y, widthRatio, heightRatio, currentUnit.getDamage()
				+ " damage", Color.RED, Tools.SMALL_SHADE, Tools.createFont("Agency FB", null, STAT_SIZE, smallestRatio));
	 ...
	 */
	public void fireBullet(final Projectile bullet, final double endpointX, final double endpointY, final double angle)
	{
		prevTime = -1;
		attacking = true;
	...
	public void genMoney()
	{
		...
		do
		{
			randomValue = (int) (Math.random() * (Tools.MAX_MONEY_BAG - MIN_MONEY_SPAWN + 1)) + MIN_MONEY_SPAWN;
			randomX = Math.random() * (GAME_PANE_WIDTH - Tools.MAX_MONEY_BAG / 2);
			randomY = Math.random() * (GAME_PANE_HEIGHT - BASE_HEIGHT * 2 - Tools.MAX_MONEY_BAG / 2) + BASE_HEIGHT;
		...
		MoneyBag spawn = Tools.createMoneyBag(randomValue, 0, 0, widthRatio, heightRatio, smallestRatio, Tools.MEDIUM_OUT_SHADE);
	 ...
	 */
	public void moveElement(UIElement ele, double endpointX, double endpointY, double angle)
	{
		ele.setLayoutX(endpointX);
		ele.setLayoutY(endpointY);

		ele.updateCollShape(ele.getFitHeight(), ele.getFitWidth(), angle, ele.getLayoutX() + ele.getFitWidth() / 2, ele.getLayoutY()
	...
	public void refreshSelect()
	{
		...
		root.getChildren().removeAll(selectIcon, selectType, selectHealthBar, selectHealthText, selectAttRange, selectMoveRange, selectDamage, attackButton, attackText, moveButton, moveText, skipButton, skipText);
		currentUnit = null;
	}
	...
	public void setCoverPane()
	{
		...
		coverPane.setOnMouseMoved(new EventHandler<MouseEvent>()
		{
			...
			public void handle(MouseEvent event)
			{
				...
				attackEllipse.setFill(BAD_GRADIENT_CIRCLE);
			}
		});
	...
	public void updateInfo()
	{
		...
		playerName = Tools.createText(PLAYER_NAME_X, PLAYER_NAME_Y, widthRatio, heightRatio, players.get(currentPlayer).getName(), PLAYER_NAME, Tools.SMALL_SHADE, Tools.createFont("Agency FB", null, NAME_SIZE, smallestRatio));

		playerMoney = Tools.createText(PLAYER_MONEY_X, PLAYER_MONEY_Y, widthRatio, heightRatio, "$"
				+ players.get(currentPlayer).getAmountOfMoney(), PLAYER_MONEY, Tools.SMALL_SHADE, Tools.createFont("Agency FB", null, MONEY_SIZE, smallestRatio));
	...
	private ArrayList<UIElement> getIntersections(UIElement test, Shape collShape)
	{
		...
		for (Node child : gamePane.getChildren())
			if (child instanceof UIElement && test != child)
			{
				...
				if (((Path) Shape.intersect(check.getCollShape(), collShape)).getElements().size() > 0)
					colls.add(check);
